...
const uint32_t ROW_SIZE = ID_SIZE + USERNAME_SIZE + EMAIL_SIZE; // 每行数据占用大小
...
const uint32_t PAGE_SIZE = 4096; // 定义每page大小
...
void cursor_advance(Cursor *cursor)
{
    ...
    {
        /**
         * 每当我们想将光标前移到叶节点的末尾时，我们都可以检查叶节点是否有同级节点。如果是，请跳到它。否则，我们就排在了桌子的末尾。
         */
...
/**
 * 获取缓存
 * 假设页面在数据库文件中一个接一个地保存：第 0 页位于偏移量 0，第 1 页位于偏移量 4096，第 2 页位于偏移量 8192，依此类推。
 * 如果请求的页面位于文件边界之外，我们知道它应该是空白的，所以我们只需分配一些内存并返回它。稍后将缓存刷新到磁盘时，该页面将添加到文件中。
 */
...
void leaf_node_split_and_insert(Cursor *cursor, uint32_t key, Row *value)
{
    ...
    *node_parent(new_node) = *node_parent(old_node);
    ...
    /**
     * 每当我们拆分叶节点时，请更新同级指针。旧叶的兄弟姐妹变成新叶子，新叶子的兄弟姐妹变成了旧叶子的兄弟姐妹。
     */
